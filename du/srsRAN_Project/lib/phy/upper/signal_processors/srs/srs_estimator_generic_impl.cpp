...
[[maybe_unused]] static uint64_t get_current_timestamp_us()
{
...
void srsran::srs_estimator_generic_impl::compensate_phase_shift(span<cf_t> mean_lse,
  ...
  for (unsigned i_antenna_port = 0; i_antenna_port != nof_antenna_ports; ++i_antenna_port) {
    ...
    for (unsigned i_rx_port_index = 0; i_rx_port_index != nof_rx_ports; ++i_rx_port_index) {
      ...
      for (unsigned i_symbol     = config.resource.start_symbol.value(),
        ...
        // Forward SRS samples to E2 collection on first SRS symbol only to avoid duplicates.
        // Note: E2 collector ingestion must not depend on e2_callback_ being configured.
        if (i_symbol == config.resource.start_symbol.value() && config.context.has_value()) {
          uint64_t timestamp_us = get_current_timestamp_us();

          const uint32_t rnti = static_cast<uint32_t>(to_value(config.context->get_rnti()));
          if (e2_callback_) {
            e2_callback_(rnti, i_rx_port, sequence, span<const cf_t>(rx_sequence), timestamp_us);
          }

          if (auto* collector = get_srs_e2_data_collector()) {
            collector->add_srs_data(rnti, i_rx_port, sequence, span<const cf_t>(rx_sequence), timestamp_us);
          }
        }
